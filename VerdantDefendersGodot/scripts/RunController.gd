		...
		if screen.has_method("setup"):
			...
			if return_context == "shop":
				if screen.has_signal("card_selected"):
					if not screen.is_connected("card_selected", _on_deck_card_selected_for_shop):
						screen.card_selected.connect(_on_deck_card_selected_for_shop)
				if screen.has_signal("cancelled"):
					if not screen.is_connected("cancelled", goto_shop):
						screen.cancelled.connect(goto_shop)
			elif return_context == "map":
				if screen.has_signal("cancelled"):
					if not screen.is_connected("cancelled", return_to_map_view):
				...
				if mode in ["remove", "sell", "sacrifice_sigil"]:
					if screen.has_signal("card_selected"):
						if not screen.is_connected("card_selected", _on_deck_card_selected_for_event):
