...
func choose_node(layer_idx: int, node_idx: int) -> void:
	# Validate move
	if layer_idx != current_layer and layer_idx != 0:
	...
	if current_node_index != -1:
		if current_layer < layers.size():
			var curr_node = _find_node_by_index(layers[current_layer], current_node_index)
			if curr_node and not node_idx in curr_node.get("next", []):
	...
	var node_data = _find_node_by_index(layers[layer_idx], node_idx)
	if not node_data:
		print("Invalid move: Node not found at layer %d index %d" % [layer_idx, node_idx])
		return
...
func _find_node_by_index(layer_nodes: Array, idx: int):
...
func _enter_room(room: Dictionary) -> void:
	...
	var type = room.get("type", "fight").to_lower()
	
	# Normalize node types to room types
	if type == "skirmish":
		type = "fight"
	elif type == "miniboss_gate" or type == "miniboss_opt":
		type = "elite"
	elif type == "start":
		type = "fight"  # First room is a fight
