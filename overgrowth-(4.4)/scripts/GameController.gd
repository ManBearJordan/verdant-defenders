...
func _ready() -> void:
	...
	if ts:
		# check if signals exist before connecting to avoid errors
		if not is_connected("turn_ended", ts.on_turn_end):
...
func start_new_run(class_id: String, run_seed: int = 0) -> void:
	...
	var deck_manager: Node = get_node_or_null("/root/DeckManager")
	var starting_deck = []
	if deck_manager: starting_deck = deck_manager.get_deck_list()
	
	var ts = get_node_or_null("/root/TelemetrySystem")
	if ts: ts.log_run_start(run_seed, class_id, 1, starting_deck, [], [])
...
func start_combat() -> void:
	...
	var dc = get_node_or_null("/root/DungeonController")Apply start of combat relic hooks
...
func play_card(idx: int, target_index: int = -1) -> bool:
	...
	if card_rules and card_rules.has_method("apply_effects"):
		...
		var cs = get_node_or_null("/root/CombatSystem")
		if cs: cs._current_card_context = card
		...
		if cs: cs._current_card_context = null
