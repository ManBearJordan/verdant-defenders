...
func _apply_outcome(outcome: Dictionary) -> void:
	...
	if "cost_hp" in outcome:
		...
		if game_controller:
			game_controller.player_hp = max(1, game_controller.player_hp - cost)
			
	if type == "heal":
		var amt = int(outcome.get("amount", 0))
		if game_controller:
			game_controller.player_hp = min(game_controller.max_hp, game_controller.player_hp + amt)
		...
		if res == "gold" or res == "shards":
			if game_controller:
				game_controller.verdant_shards += amt
