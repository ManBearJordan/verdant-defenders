...
func _on_map_updated(map_data, layer, idx): _show_map(map_data, layer, idx)
func _show_map(map_data, layer, idx):
	...
	_map_screen_instance.visible = true
...
func _on_map_node_selected(l, i, t):
	_map_screen_instance.visible = false
	var dc = get_node_or_null("/root/DungeonController")
...
func _on_room_entered(room):
	if enemies_box: _clear_box(enemies_box)
	if hand_box: _clear_box(hand_box)
	if _map_screen_instance: _map_screen_instance.visible = false
	var type = room.get("type", "")
	if type in ["fight", "elite", "boss"]: _spawn_enemies_in_ui()
	elif type == "rest": pass # Implement rest
...
func _on_menu_requested() -> void:
	...
	if ResourceLoader.exists(ps):
		get_tree().change_scene_to_file(ps)
	else:
		get_tree().quit()
