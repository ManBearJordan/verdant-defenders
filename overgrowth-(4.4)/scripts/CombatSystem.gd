		...
		if gc:
			...
			if seeds >= threshold:
				...
				if e.custom_data["seed_harvest_streak"] >= 2:
					...
					# Reset streak? Spec doesn't say. "If triggers 2 turns in a row". 
					# Usually streak implies it keeps triggering.
					# But maybe reset to require another 2? Let's keep it punishing.
			else:
				# Reset streak if safe
				e.custom_data["seed_harvest_streak"] = 0
				e.set_status("temp_strength_decay", 1) # Legacy? Remove if not in spec. 
				# Spec says "Boss gains +2 Strength (stacking)". No temporary decay mentioned.
				# So permanent strength.
...
func process_turn_end_effects() -> void:
	...
	for i in range(enemies.size()):
		...
		if e.current_hp <= 0:
			...
			_trigger_boss_mechanic(e, "on_damage_taken", {"source": "poison", "amount": poison_dmg})
			_check_elite_hooks("on_poison_tick", e, {"damage": poison_dmg})
